...
class LinksControllerIntegrationTest : AbstractSpringIntegrationTest() {
    ...
    fun `anonymous user`() {
        mvc.perform(get("/v1/"))
            ...
            .andExpect(jsonPath("$._links.currentUser").doesNotExist())
    ...
    fun `registered user`() {
        ...
        mvc.perform(get("/v1/").asUser("a-user-id"))
            ...
            .andExpect(jsonPath("$._links.currentUser").exists())
    ...
    fun `registered lifetime user`() {
        ...
        val organisationAccount = organisationRepository.save(OrganisationFactory.school())
        userRepository.create(
            UserFactory.sample(
                identity = IdentityFactory.sample(id = "a-user-id"),
                profile = ProfileFactory.sample(),
